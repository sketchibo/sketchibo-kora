...
def handle_action(action, mode: str) -> str:
    ...
    if intent == "run_shell":
        ...
        if not cmd:
            return "ACTION_ERROR: empty command"
        out = safe_cmd(cmd)
        return out if out.strip() else "OK"

    return f"ACTION_UNKNOWN: {intent}"
...
def main():
    ...
    while True:
        ...
        action = interpret(u)
        if action.get("mode") == "action":
            action_out = handle_action(action, mode)
            print("\nKORA:", action_out)
            speak(action_out)
            continue
